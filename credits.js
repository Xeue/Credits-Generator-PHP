  ...
  switch (property) {
    ...
    case "image":
      ...
      if (typeof block[property] == "object") {
        ...
        for (var i = 0; i < block[property].length; i++) {
          subHtml += "<img class='image' src='saves/"+$("#loadFile").find(":selected").val()+"/"+block[property][i]+"' style='max-height: "+height+"vh'>";
        }
        subHtml += "</div>";
      } else {
        subHtml += "<img class='image' src='saves/"+$("#loadFile").find(":selected").val()+"/"+block[property]+"' style='max-height: "+height+"vh'>";
      }
      break;
    case "spacing":
